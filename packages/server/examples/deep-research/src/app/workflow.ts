...
// workflow factory
export const workflowFactory = async (reqBody: unknown) => {
  const index = await VectorStoreIndex.fromDocuments([
    new Document({
      text: `Google offers a wide range of cloud services through Google Cloud Platform (GCP). Their key services include:
    - Google Compute Engine for virtual machines and cloud computing
    - Google Cloud Storage for object storage and file hosting
    ...
    - Google Workspace (formerly G Suite) for productivity and collaboration tools including Gmail, Drive, Docs, Sheets etc.
    Google's cloud infrastructure spans globally with data centers in multiple regions, offering high availability and low latency.`,
    }),
    new Document({
      text: `Apple provides various digital services and platforms including:
    - iCloud for cloud storage, backup and syncing across Apple devices
    - App Store for iOS and macOS applications
    - Apple Music for music streaming and digital music purchases
    - Apple TV+ for original video streaming content
    - Apple Arcade for gaming subscription service
    - iMessage for messaging between Apple devices
    - Apple Pay for digital payments and mobile wallet
    - iWork suite (Pages, Numbers, Keynote) for productivity
    - Apple Maps for navigation and location services
    Apple's services are tightly integrated with their hardware ecosystem of iPhones, iPads, Macs and other devices, creating a seamless user experience.`,
    }),
  ]);
...
const planResearchEvent = workflowEvent<object>();
const researchEvent = workflowEvent<ResearchQuestion>();
const reportEvent = workflowEvent<object>();
