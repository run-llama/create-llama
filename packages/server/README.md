...
```
next/ -- this folder is generated by pnpm eject
src/
  index.ts
  ...
```

To start the server in eject mode, set the `EJECT` environment variable to `true`, then run `pnpm run dev`.

```bash
export EJECT=true pnpm dev
pnpm run dev
```
